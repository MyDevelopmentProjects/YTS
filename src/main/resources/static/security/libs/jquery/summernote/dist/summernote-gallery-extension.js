...
(function ($) {
    ...
    var showGalleryDialog = function ($editable, $dialog, body) {
        return $.Deferred(function (deferred) {
            var $galleryDialog = $dialog.find('.note-gallery-dialog');
            var $galleryComp   = $dialog.find('.smrnt-image-gallery');

            $galleryDialog
                .one('shown.bs.modal', function () {
                    $galleryComp.html(body);
    ...
    var loadImages = function ($editble, $dialog, url, callback) {
                ...
                if(response.length > 0){
                    for(var i in response){
                        html += '<div class="col-md-4 gallery-item">'+
                                    '<div class="box p-a-x-s">'+
                                        '<img src='+response[i]+' class="img-responsive">'+
                                    '</div>';
    ...
    $.summernote.addPlugin({
        ...
        dialogs: {
            gallery: function (lang) {
                var body = '<div class="row"><div class="smrnt-image-gallery"></div>';
                var footer = '<button href="#" class="btn btn-primary note-gallery-save disabled" disabled>' + lang.gallery.insert + '</button>';
                return tmpl.dialog('note-gallery-dialog', lang.gallery.title, body, footer);
            }
        },
        events: {
            showGalleryDialog: function (event, editor, layoutInfo) {
                var $dialog = layoutInfo.dialog(),
                    $editable = layoutInfo.editable();
                // save current range
                editor.saveRange($editable);
                loadImages($editable, $dialog,"/upload",function (html) {
                    showGalleryDialog($editable, $dialog, html).then(function (url) {
            ...
            'gallery': function (context) {
                ...
                this.events = {
                    ...
                    'summernote.init': function (we, e) {
                        ...
                        self.setModalHtml = function (html) {   // set variabl parts to modal html
